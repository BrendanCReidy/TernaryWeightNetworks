...
class Block(nn.Module):
    ...
    def forward(self, x):
        ...
        #else:
            #x = F.avg_pool2d(x, 2, stride=self.stride)
            #out = out + self.shortcut(x)
        return out
...
class TernaryMobileNetV2(nn.Module):
    ...
    def __init__(self, num_classes=10, ternarized=False):
        ...
        self.bn1 = nn.BatchNorm2d(32)
        self.layers = self._make_layers(in_planes=32)
        self.conv2 = TernaryConv2d(320, 256, kernel_size=1, stride=1, padding=0, bias=False)
        self.bn2 = nn.BatchNorm2d(256)
        self.linear = TernaryLinear(256, num_classes)
